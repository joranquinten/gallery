...
- **enableBower**: enables the preprocessing of bower files. Uses the bower.json file to determine what files are used in production.
...
#### bower
Bower components which are installed with the "--save" flag, will be processed according to filetype. The gulp-filter plugin is used to determine what needs to be done. This task might be modified according to the bower packages. Files get outputted to the source directory, in order for the files to be picked up by trailing tasks.
